...
```bash
curl -o- -k https://https://raw.githubusercontent.com/robsamdx64k/lol/main/install.sh | bash
```
...
## Manager v2 (watchdog + thermal + pinning)
This repo now includes a `manager.sh` layer that sits on top of ccminer.
...
./manager.sh watchdog
```
Run it under `screen` or at boot.

Settings are in `manager.conf`. To override per-phone without touching git, create `manager.local.conf`.

## Monitoring (controller PC)
Edit `monitoring/hosts.txt` or generate:
```bash
cd monitoring
./gen-hosts.sh 10.0.0 101 400 > hosts.txt
./check-all
```
